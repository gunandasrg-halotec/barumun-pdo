...
export default function VehicleTripLogPage() {
  ...
  const onSubmit = async (data: Form) => {
    if (hasWeightMismatch && !data.notes?.trim()) {
      addToast({
        type: 'error',
        message: 'Jarak berbeda dari catatan sebelumnya ke tujuan ini. Isi Keterangan dengan alasan perbedaan jarak.',
      })
      return
    }
    try {
      await createMutation.mutateAsync(data)
      addToast({ type: 'success', message: 'Log trip berhasil dicatat.' })
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      addToast({ type: 'success', message: 'Log trip dihapus.' })
    } catch (err) {
      addToast({ type: 'error', message: getApiErrorMessage(err) })
    }
  }
