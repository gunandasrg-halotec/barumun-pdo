...
type Props = {
  detailId: string
  canUpload: boolean  // Kerani + draft
  colSpan: number
}
...
export function DetailAttachmentPanel({ detailId, canUpload, colSpan }: Props) {
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['detail-attachment-count', detailId] })
      ...
      <td colSpan={colSpan} className="px-0 py-0 border-t border-dashed border-line bg-[#f9fbf9]">
        <div className="pl-10 pr-4 py-3">
          <div className="flex items-center gap-3 mb-2">
            <span className="text-[11px] font-[850] uppercase tracking-wider text-muted flex items-center gap-1.5">
              <Paperclip className="w-3.5 h-3.5" /> Lampiran
            </span>
            {canUpload && (
              <>
                <button
...
// Badge kecil untuk jumlah lampiran di kolom tabel
